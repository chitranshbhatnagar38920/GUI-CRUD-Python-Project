...
def updateARow(rowDataList,param): #rowDataList is a series
        ...
        try:
                ...
                for colname in rowDataList.index:
                        ...
                        Label(frameUpdateTable, text=colname, bg='yellow', fg='black') \
...
def extra():
    ...
    def insertIntoTableButtonClickEvent():
           ...
           try:
                   ...
                   frameInsertIntoTable.configure(background='yellow')
                   total_rows = df.shape[0]
                   r = 0
                   for i in range(total_rows):
                           '''
                           colname = df[i]
                           Label(frameInsertIntoTable, text=colname, bg='yellow', fg='black') \
                                                           .grid(row=r, column=1, padx=15, pady=5, sticky='E')
...
def createInsertDataInputFRM(insertframe, table, param={}, r=0, c=1):
        ...
        insertDataInputFRM.configure(background='yellow')
        try:
                   ...
                   for i in range(total_rows):
                           '''
                           colname = df[i]
                           Label(insertDataInputFRM, text=colname, bg='yellow', fg='black') \
                                                           .grid(row=r, column=1, padx=15, pady=5, sticky='E')
...
def createInsertChildFrames(insertrootframe, param):
        ...
        Label(frame, text='TABLE:', bg='black', fg='white').grid(row=0, column=0, sticky=W)
        Label(frame, text=table, bg='black', fg='white').grid(row=0, column=1, sticky=W)
        Label(frame, text='PRIMARY KEY:', bg='black', fg='white').grid(row=1, column=0, sticky=W)
        Label(frame, text=pk, bg='black', fg='white').grid(row=1, column=1, sticky=W)
        Label(frame, text='LOOKUP VALUES (CBO):', bg='black', fg='white').grid(row=2, column=0, sticky=W)
        Label(frame, text=cbos, bg='black', fg='white').grid(row=2, column=1, sticky=W)
        frame.configure(background="black",bd=-10)
...
def createInsertRootFrame(rootframe, param):
        ...
        insertrootframe.configure(background="black",bd=-10)
